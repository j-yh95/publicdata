...
<head>
	...
	#roadview {position:absolute; left:0px; bottom:0px; width:100%; height:100%; position:absolute; z-index:1;}
	#roadviewControl {position:absolute;top:10px;left:10px;width:42px;height:42px;z-index: 7;cursor: pointer; background: url(https://t1.daumcdn.net/localimg/localimages/07/2018/pc/common/img_search.png) 0 -450px no-repeat;}
	#roadviewControl.active {background-position:0 -350px;}

	#map {position:absolute; overflow:auto; bottom:0px; left:0px; width:500px; height:400px; z-index:2; border:1px solid #909090;box-shadow: 0 1px 1px rgba(0, 0, 0, 0.4);border-radius: 5px;margin:0;padding:0;} /*visibility: hidden;*/
	#rsz {position:absolute;right: 0;top: 0;width:36px;height:36px;background:url(https://t1.daumcdn.net/localimg/localimages/07/2018/pc/roadview/ico_roadview22x_190625.png) -100px -181px no-repeat;background-size:200px auto;z-index: 3; cursor:nesw-resize;}

	#mapChanger {padding-left:45px; display:inline; float:left;}
	#markerLoading {padding-right:45px; display:inline; float:right;}

	/* 스카이뷰 , 지도 전환  */
	.radius_border{border:1px solid #919191;border-radius:5px;}
	.custom_typecontrol {position:absolute;top:10px;left:62px;overflow:hidden;width:197px;height:32px;margin:0;padding:0;z-index:7;font-size:12px;font-family:'Malgun Gothic', '맑은 고딕', sans-serif;}
	.custom_typecontrol span {display:block;width:65px;height:30px;float:left;text-align:center;line-height:30px;cursor:pointer;}
	.custom_typecontrol .btn {background:#fff;background:linear-gradient(#fff,  #e6e6e6); border:0;margin:0;padding:0;z-index:7;font-size:12px;font-family:'Malgun Gothic', '맑은 고딕', sans-serif;}       
	...
	/* 지도 확대 */
	.custom_zoomcontrol {position:absolute;top:55px;left:13px;width:36px;height:80px;overflow:hidden;z-index:7;background-color:#f5f5f5;} 
	.custom_zoomcontrol span {display:block;width:36px;height:40px;text-align:center;cursor:pointer;}
	.custom_zoomcontrol span img {width:15px;height:39px; padding-bottom:12px; padding-top:12px; border:none;} 
	.custom_zoomcontrol span:first-child{border-bottom:1px solid #bfbfbf;} 

	/* 좌측 상단 메뉴 */
	input[id="menuicon"]{display:none;}
	input[id="menuicon"] + label {display:block;margin:10px;width:30px;height:30px;position:relative;cursor:pointer;}
	input[id="menuicon"] + label span {display:block;position:absolute;width:100%;height:5px;border-radius:30px;background:#000;transition:all .35s; z-index:6;}
	...
	/* 메뉴 클릭했을 때 나오는 상단 바 */
	div[class="topbar"]{width:100%;height:5%;background:#222;position:fixed;top:-300px;z-index:5;transition:all .35s;}
...
<body>
	...
	<div id="mapwrap" z-index: 0;>
		<div id="roadview"></div>
		<div id="map" style="visibility: visible;">
			<div id="roadviewControl" class="active" onclick="setRoadviewRoad()"></div>
			<div class="custom_typecontrol radius_border">
				<span id="btnRoadmap" class="selected_btn" onclick="setMapType('roadmap')">지도</span>
				<span id="btnSkyview" class="btn" onclick="setMapType('skyview')">스카이뷰</span>
				<span id="btnPolygon" class="btn" onclick="polygonControl()">경계</span>
			</div>
	...
	<div class="topbar">
		<div id="mapChanger">
			<button type="button" class="btn btn-dark btn-lg" onclick="moveKakaoRoadview()">카카오</button>
			<button type="button" class="btn btn-dark btn-lg" onclick="moveNaverStreetview()">네이버</button>
		</div>
		<div id="markerLoading">
			<button type="button" class="btn btn-dark btn-lg" onclick="createAgoMarker(0)">어제</button>
			<button type="button" class="btn btn-dark btn-lg" onclick="createAgoMarker(1)">로드뷰</button>
			<button type="button" class="btn btn-dark btn-lg" onclick="createAgoMarker(2)">마우스</button>
			<button type="button" class="btn btn-dark btn-lg" onclick="removeAgoMarker()">마커 제거</button>
		</div>
	</div> -->
</body>
...
function createPolygons(){
	...
	function displayDongArea(name, coordinates) {
		...
		polygonCustomOverlays.push(customOverlay);
	...
	function polygonControl(option){
		var documentPolygonControl = document.getElementById('btnPolygon');
		//if 폴리곤이 안 켜져있으면 생성 else 폴리곤이 켜져있으면 제거
		if (documentPolygonControl.className === 'btn'){
			documentPolygonControl.className = 'selected_btn';
			//if 폴리곤 배열이 존재한다면 폴리곤 배열에서 가져오기 else 폴리곤 배열이 없다면 createPolygons 함수 실행
			if(polygons[0] != null){
				for (var i = 0; i < polygons.length; i++) {
					polygons[i].setMap(map);
				}
				for (var i =0; i < polygonCustomOverlays.length; i++){
					polygonCustomOverlays[i].setMap(map);
				}
			} else {
				createPolygons();
			}
		...
		//폴리곤이 켜져있으면 제거
			...
			documentPolygonControl.className = 'btn';
				for (var i = 0; i < polygons.length; i++) {
					polygons[i].setMap(null);
				}
				for (var i =0; i < polygonCustomOverlays.length; i++){
					polygonCustomOverlays[i].setMap(null);
				}
			}
		}
	function zoomIn() {
	...
	function createAgoMarker(optionIndex) {
		...
		//임시 코드
		if(optionIndex == "0"){var nowYearDateTime = String(date.getFullYear()) + String(date.getMonth() + 1) + String(date.getDate() - 1);}
